...
ses <- c(
  "`Mother's age`", "`High school`", "Income", "SES", "Siblings",
  "`Time investment`", "Married"
)
ses_wo_ti <- c(
  "`Mother's age`", "`High school`", "Income", "SES", "Siblings", "Married"
)
...
mdls <- list(
  "Multinom. Logit (1)" = multinom(fmla_ib, data = df),
  "Multinom. Logit (2)" = multinom(fmla_ib_ses, data = df)
) 
...
rows <- tribble(
  ~term,   ~y.level,       ~`Multinom. Logit (1)`,  ~`Multinom. Logit (2)`,
  "", "", "", "",
  "", "", "", "",
  "", "", "", "",
  "", "", "", "",
...
attr(rows, 'position') <- c(3, 6, 9, 12, 15, 18, 21, 24)
modelsummary(
  mdls, stars = TRUE,
  estimate = "{estimate}{stars} ({std.error})",
  statistic = NULL,
  coef_omit = "(Intercept)|Income|`Mother's age`|`High school`|Married|Pos. Regard|Neg. Regard",
  coef_rename = c(
